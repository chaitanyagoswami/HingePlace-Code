...
class CancelPointsSpherical:
    ...
    def spherical_sample(self, points, centre, r1, thickness):

        focus_points = points[np.sum(np.square(points-centre), axis=1)<= r1**2].copy()
        cancel_points = points[np.sum(np.square(points-centre), axis=1)> r1**2].copy() 
        cancel_points = points[np.sum(np.square(points-centre), axis=1)<= (r1+thickness)**2].copy() 
        return focus_points, cancel_points
