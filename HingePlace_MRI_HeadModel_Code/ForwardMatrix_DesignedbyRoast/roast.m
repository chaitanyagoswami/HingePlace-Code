...
if any(~strcmpi(recipe,'leadfield'))
    ...
    capname = '10-05'; % '10-05' or 'BioSemi' or 'EGI' or 'Custom'. If using 'Custom' define the electrode configuration in capInfo.xlsx 
    electrodeLocations = readtable('capInfo.xlsx');
    electrodeNames = string(table2array(electrodeLocations(:,1)));
    electrodeLocations = table2array(electrodeLocations(:,2:4));
    centerElecName = 'Cz'; % '10-05'->Cz. The electrode at the centre of patch
    CzLocation = electrodeLocations(electrodeNames=='Cz', 1:3);
    C3Location = electrodeLocations(electrodeNames=='C3', 1:3);
    conv_ratio = sqrt(sum((C3Location-CzLocation).^2))/7; % Assuming distance between C3 and Cz is 7cm
    PatchDist = 8*conv_ratio; % Only Consider electrodes in the patch that are 8 cm from Cz
    patchElectrodeNum = 1;
    for i = 1:length(electrodeNames)
        if sqrt(sum(square(electrodeLocations(i,:)-CzLocation)))<PatchDist
            PatchElectrodes(patchElectrodeNum) = electrodeNames(i);
            patchElectrodeNum = patchElectrodeNum+1;
        end
    end
    patchElectrodeNum = patchElectrodeNum-1;
    ...
    elecOri = [];
    elecName = {};
    for i=1:length(PatchElectrodes)
        elecName{i} = convertStringsToChars(PatchElectrodes(i));
    end
    elecName{length(elecName)+1} = 'AF1';
    elecName = elecName';
    elecPara = struct('capType',capType,'elecType',elecType,...
        'elecSize',elecSize,'elecOri',elecOri) 
end
