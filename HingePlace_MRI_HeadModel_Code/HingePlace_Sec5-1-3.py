...
import os
import numpy as np
from HingePlace_optimizers import HingePlaceMultiSite_p1, SparsePlaceMultiSite, HingePlaceMultiSite_p2, HingePlaceMultiSite_p3
...
cwd = os.getcwd()
forward_mat_dir = os.path.join(cwd,'ForwardMatrix_DesignedbyRoast')
results_dir = os.path.join(cwd,'HingePlaceMRI_Results')
if not os.path.exists(results_dir):
    os.makedirs(results_dir)
base_dir = os.path.join(results_dir,'Sec-5-1-3')
...
#### Target Location and Creating Focus and Cancel Matrix
mni_target_locs = np.array([[-48, -8, 50], [48, -8, 50]])
target_locs = np.empty([mni_target_locs.shape[0],3])
idx_focus_lst, idx_cancel_lst = [], []
Af = np.empty([mni_target_locs.shape[0], Aall.shape[1]])
target_radius, target_cancel = 2, 40
for i in range(len(mni_target_locs)):
    temp = np.squeeze(np.matmul(mni2mri,np.array(list(mni_target_locs[i])+[1]).reshape(-1,1)))
    target_locs[i] = np.round(temp[0:3])
    dist_target = np.sqrt(np.sum(np.square(locations-target_locs[i].reshape(1,3)), axis=1))
    idx_focus_lst.append(np.arange(locations.shape[0])[dist_target<=target_radius])
    idx_cancel_tmp = np.arange(locations.shape[0])[dist_target<=target_cancel]
    idx_cancel_lst.append(np.setdiff1d(idx_cancel_tmp,idx_focus_lst[i]))
    Af[i] = np.mean(A_plot[idx_focus_lst[i]], axis=0)
idx_focus = np.concatenate(idx_focus_lst, axis=0)
idx_cancel = np.concatenate(idx_cancel_lst, axis=0)
A_check = A_plot[np.setdiff1d(np.arange(locations.shape[0]),idx_focus)]
locations_cancel = locations[np.setdiff1d(np.arange(locations.shape[0]),idx_focus)]

tmp = np.setdiff1d(np.arange(locations.shape[0]), idx_cancel)
tmp = np.setdiff1d(tmp, idx_focus)
...
#### Hyperparamaters
Edes = np.array([1,1]) ## V/m
Isafety_lst = [4.4,4.8,5.2,5.6,6,6.4] ## mA
Itot_mul = [4,8]
Etol = [0.1*np.max(Edes), 0.5*np.max(Edes), 0.6*np.max(Edes), 0.7*np.max(Edes)] ## p=1
Etol_HP2 = [0.65*np.max(Edes), 0.55*np.max(Edes), 0.35*np.max(Edes), 0.01*np.max(Edes)] ## p=2
Etol_HP3 = [0.65*np.max(Edes), 0.55*np.max(Edes), 0.35*np.max(Edes), 0.01*np.max(Edes)] ## p=3
    ...
    for ii in range(len(Isafety_lst)):
        ...
        for kk in range(len(Etol_HP2)): ### Brute-force search to find the best possible Etol value
            if not LOAD_FLAG_HP2:
                I_HP2_tmp = HingePlaceMultiSite_p2(Jdes=Edes, Isafety=Isafety, Jtol=Etol_HP2[kk], Itotal=Itotal, Af=Af, Ac=Ac)
            ...
            if I_HP2_tmp is not None and np.sum(np.abs(np.matmul(A_check, I_HP2_tmp)) > 0.8 * np.max(Edes)) < area_HP2:
                I_HP2 = I_HP2_tmp.copy()
                area_HP2 = np.sum(np.abs(np.matmul(A_check, I_HP2)) > 0.8 * np.max(Edes))
        ...
        if PLOT_EFIELD_AND_ACTIVATION_HP2 and area_HP2 != -1: #### Plot the electric field and activated region of the best performing Etol value
            ...
            plot_electric_field(Efield=efield_HP2, locations=locations, savepath=os.path.join(savedir_hp,"Efield_I_HP2_Isafety%dmA_Etol%d.png" % (int(Isafety * 10),kk + 1)))
            plot_activation(Efield=efield_off_target_HP2, locations_cancel=locations_cancel, locations=locations,savepath=os.path.join(savedir_hp, "Activation_I_HP2_Isafety%dmA_Etol%d.png" % (int(Isafety * 10), kk + 1)))
        ...
        for kk in range(len(Etol)): ### Brute-force search to find the best possible Etol value
            if not LOAD_FLAG_HP2:
                I_HP_tmp = HingePlaceMultiSite_p1(Jdes=Edes, Isafety=Isafety, Jtol=Etol[kk], Itotal=Itotal, Af=Af, Ac=Ac)
            ...
            if I_HP_tmp is not None and np.sum(np.abs(np.matmul(A_check, I_HP_tmp)) > 0.8 * np.max(Edes)) < area_HP:
                I_HP = I_HP_tmp.copy()
                area_HP = np.sum(np.abs(np.matmul(A_check, I_HP)) > 0.8 * np.max(Edes))
        ...
        for kk in range(len(Etol_HP3)):
            if not LOAD_FLAG_HP3:
                I_HP3_tmp = HingePlaceMultiSite_p3(Jdes=Edes, Isafety=Isafety, Jtol=Etol_HP3[kk], Itotal=Itotal, Af=Af, Ac=Ac)
            ...
            if I_HP3_tmp is not None and np.sum(np.abs(np.matmul(A_check, I_HP3_tmp)) > 0.8 * np.max(Edes)) < area_HP3:
                area_HP3 = np.sum(np.abs(np.matmul(A_check, I_HP3_tmp)) > 0.8 * np.max(Edes))
        ...
        LOAD_FLAG_SP = False
        if not LOAD_FLAG_SP:
            I_SP = SparsePlaceMultiSite(Jdes=Edes, Isafety=Isafety, Itotal=Itotal, Af=Af, Ac=Ac)(Jdes=Edes, Isafety=Isafety, Itotal=Itotal, Af=Af, Ac=Ac)
            if I_SP is not None:
                area_SP = np.sum(np.abs(np.matmul(A_check, I_SP)) > 0.8 * np.max(Edes))
...
plt.ylim(ymax=60)
plt.legend(fontsize=17, ncols=2, loc="upper left")
plt.xlabel(r"$I_{safe}$ (mA)", fontsize=21)
