...
results_dir = os.path.join(cwd,'HingePlaceMRI_Results/Sec-5-1-4')
...
#### List of different target points
mni_target_locs = np.array([[-48, -21, 50], [-22,-20,77], [5,8,61], [48, -8, 50]])
...
for i in range(len(mni_target_locs)):
    ...
    target_locs_lst[i] = np.round(temp[0:3])
target_radius_lst = [2,3,2,2]

for l in range(len(target_locs_lst)):
    print(">>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print(">>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print(">>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print("Running Simulation for Target %d....." % (l+1))
    print(">>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print(">>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print(">>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    base_dir = os.path.join(results_dir, 'DiffTarget%d' % (l + 1))
    if not os.path.exists(base_dir):
        os.makedirs(base_dir)
    target_loc = target_locs_lst[l].reshape(1,3)

    #### Creating Focus and Cancel Matrix
    dist_target = np.sqrt(np.sum(np.square(locations - target_loc.reshape(1, 3)), axis=1))
    target_radius = target_radius_lst[l]
    ...
    Edes = 1 ## V/m
    if l==0:
        Isafety_lst = np.linspace(4,5,5) ## mA
    elif l==1:
        Isafety_lst = np.linspace(10,13,5) ## mA
    elif l==2:
        Isafety_lst = np.linspace(8,10,5)  ## mA
    elif l==3:
        Isafety_lst = np.linspace(6,7.5,5)  ## mA

    Itot_mul = [2,8]
    rel_inc_HP, rel_inc_HP2, rel_inc_HP3, Isafe = [], [], [], []
    Etol = [0.7 * Edes, 0.6 * Edes, 0.5 * Edes]
    Etol_HP2 = [0.65 * Edes, 0.55 * Edes, 0.3 * Edes]
    Etol_HP3 = [0.65 * Edes, 0.55 * Edes, 0.3 * Edes]
    for i in range(len(Itot_mul)):
        ...
        for ii in range(len(Isafety_lst)):
            ...
            PLOT_EFIELD_AND_ACTIVATION_HP2 = True
            if PLOT_EFIELD_AND_ACTIVATION_HP2 and area_HP2 != -1: #### Plot the electric field and activated region of the best performing Etol value
                ...
                plot_electric_field(Efield=efield_HP2, locations=locations, savepath=os.path.join(savedir_hp,"Efield_I_HP2_Isafety%dmA_Etol%d.npy" % (int(Isafety * 10),kk + 1)))
                plot_activation(Efield=efield_off_target_HP2, locations_cancel=locations_cancel, locations=locations,savepath=os.path.join(savedir_hp, "Activation_I_HP2_Isafety%dmA_Etol%d.npy" % (int(Isafety * 10), kk + 1)))
            ...
            if PLOT_EFIELD_AND_ACTIVATION_HP and area_HP != -1: #### Plot the electric field and activated region of the best performing Etol value
                ...
                plot_electric_field(Efield=efield_HP, locations=locations, savepath=os.path.join(savedir_hp,"Efield_I_HP_Isafety%dmA_Etol%d.npy" % (int(Isafety * 10),kk + 1)))
                plot_activation(Efield=efield_off_target_HP, locations_cancel=locations_cancel, locations=locations,savepath=os.path.join(savedir_hp, "Activation_I_HP_Isafety%dmA_Etol%d.npy" % (int(Isafety * 10), kk + 1)))
            ...
            if PLOT_EFIELD_AND_ACTIVATION_HP3 and area_HP3 != -1: #### Plot the electric field and activated region of the best performing Etol value
                ...
                plot_electric_field(Efield=efield_HP3, locations=locations, savepath=os.path.join(savedir_hp,"Efield_I_HP3_Isafety%dmA_Etol%d.npy" % (int(Isafety * 10),kk + 1)))
                plot_activation(Efield=efield_off_target_HP3, locations_cancel=locations_cancel, locations=locations,savepath=os.path.join(savedir_hp, "Activation_I_HP3_Isafety%dmA_Etol%d.npy" % (int(Isafety * 10), kk + 1)))
        ...
        plt.plot(np.array(Isafety_lst)[activ_SP!=-1], activ_SP[activ_SP!=-1], marker='x', label='LCMV-E')
